...
from Modelos.modelo_FlujoDC_con_reactiva_modif.FlujoDC_con_reactiva_modif import FlujoDC_con_reactiva_v2
...
flujodc_novedoso        = FlujoDC_con_reactiva()
flujodc_novedoso_v2       = FlujoDC_con_reactiva_v2()
...
flujodc_novedoso_v2.fit(Red)                               # Aplico modelo flujodc novedoso
...
Sol_novedoso_v2            = flujodc_novedoso_v2.predict(Red.potencias,Red.voltajes)
flujo_ac.predict()
...
lista_de_modelos = [['Flujo AC start','Flujo DC Clasico' , 'Flujo DC psse','Flujo DC con reactiva','Flujo DC con reactiva v2','Flujo DC psse Modif','X/R','Error absoluto clásico - AC','Error absoluto DC psse - AC','Error absoluto DC con reactiva - AC','Error absoluto DC con reactiva v2 - AC','Error absoluto DC psse Modif - AC']]
#lista_de_modelos = [['Flujo AC end','Flujo AC start','Flujo DC Clasico']]
n_modelos=6
cantidad_ramas = len(Red.ramas)

Matriz_de_flujos = np.zeros((cantidad_ramas,len(lista_de_modelos[0])))
# print('******************************************')
...
Matriz_de_flujos[:,lista_de_modelos[0].index('Flujo DC con reactiva v2')]=np.around(Sol_novedoso_v2,3)  
Matriz_de_flujos[:,lista_de_modelos[0].index('Flujo AC start')]=flujo_ac.resultado_ac['p_branches_start']
#Matriz_de_flujos[:,lista_de_modelos[0].index('Flujo AC start')]=flujo_ac.resultado_ac['p_branches_start']

#Matriz_de_flujos[:,lista_de_modelos[0].index('X/R')]=Red.ramas['X (pu)'][:Red.numero_ramas]/Red.ramas['R (pu)'][:Red.numero_ramas]
#Matriz_de_flujos[:,lista_de_modelos[0].index('Error relativo clásico - AC')] = abs(np.divide(flujo_ptdf_clasico - flujo_ac.p_branches_end,flujo_ac.p_branches_end)*100)
#Matriz_de_flujos[:,lista_de_modelos[0].index('Error relativo DC psse - AC')] = abs(np.divide(flujo_ptdf_psse - flujo_ac.p_branches_end,flujo_ac.p_branches_end)*100)
#Matriz_de_flujos[:,lista_de_modelos[0].index('Error relativo DC psse Modif - AC')] = abs(np.divide(flujo_ptdf_psse_modificado - flujo_ac.p_branches_end,flujo_ac.p_branches_end)*100)
#Matriz_de_flujos[:,lista_de_modelos[0].index('Error relativo DC con reactiva - AC')] = abs(np.divide(np.around(Sol_novedoso) - flujo_ac.p_branches_end,flujo_ac.p_branches_end)*100)

Matriz_de_flujos[:,lista_de_modelos[0].index('Error absoluto clásico - AC')] = abs(flujo_ptdf_clasico - flujo_ac.resultado_ac['p_branches_start'])
Matriz_de_flujos[:,lista_de_modelos[0].index('Error absoluto DC psse - AC')] = abs(flujo_ptdf_psse - flujo_ac.resultado_ac['p_branches_start'])
Matriz_de_flujos[:,lista_de_modelos[0].index('Error absoluto DC psse Modif - AC')] = abs(flujo_ptdf_psse_modificado - flujo_ac.resultado_ac['p_branches_start'])
Matriz_de_flujos[:,lista_de_modelos[0].index('Error absoluto DC con reactiva - AC')] = abs(np.around(Sol_novedoso) - flujo_ac.resultado_ac['p_branches_start'])
Matriz_de_flujos[:,lista_de_modelos[0].index('Error absoluto DC con reactiva v2 - AC')] = abs(np.around(Sol_novedoso_v2 - flujo_ac.resultado_ac['p_branches_start']))
#print('*************************************************')
