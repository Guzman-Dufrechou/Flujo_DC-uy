...
def calcular_flujo(self,delta_modif, G, B, lambda1):
    ...
    for rama in self.ramas.index:
        ...
        flujo[rama] = ((G[i,j]/(lambda1*2))*(delta_modif[i]-delta_modif[j])**2 + (B[i,j]/lambda1)*(delta_modif[i]-delta_modif[j]))/self.contador[i,j]
        #flujo[rama] = (B[i,j]/lambda1)*(delta_modif[i]-delta_modif[j])
        
        #print('Para la rama: ', ramas['FROMNUMBER'][rama],'/',net.ramas['TONUMBER'][rama], ' (',rama,') ', ' el delta i: ', delta_modif[i] , ' el delta j: ', delta_modif[j])
    return flujo